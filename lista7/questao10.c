...
Escreva uma função que retorna verdadeiro se uma árvore binária é uma árvore binária de busca e
...
int verifica(ArvBin * raiz){
    ...
    while (atual != NULL || topo != -1) {
        ...
        if (ultimo_no != NULL && atual->info <= ultimo_no->info) return 0; // Não é ABB
        ultimo_no = atual;
        atual = atual->dir;
    }
    return 1; // Eh ABB
