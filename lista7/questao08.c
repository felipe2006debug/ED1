...
struct NO * removePar(struct NO * no){
  if(no == NULL) return NULL;

  no->esq = removePar(no->esq);
  no->dir = removePar(no->dir);

  if(no->info % 2 == 0){
    struct NO * filhoesq = no->esq;
    struct NO * filhodir = no->dir;

    if(filhoesq != NULL){
      struct NO * temp = filhoesq;
      while(temp->dir != NULL)
        temp = temp->dir;
...
int main(){
  ...
  insereBalanceado(raiz, 67);
  insereBalanceado(raiz, 23);
  insereBalanceado(raiz, 1);
  imprime(raiz);
