...
Implemente a versão iterativa do algoritmo de remoção de uma árvore AVL.
*/
...
int removeIterativo(ArvAVL *raiz, int valor) {
    if (raiz == NULL || *raiz == NULL) {
        printf("Valor não existe!!\n");
        return 0;
    }
    ...
    if (atual->esq != NULL && atual->dir != NULL) {
        struct NO* pai_sucessor = atual;
        struct NO* sucessor = atual->dir;
        
        caminho[topo++] = atual; 

        while (sucessor->esq != NULL) {
            caminho[topo++] = sucessor;
    ...
    for (int i = topo - 1; i >= 0; i--) {
        ...
        if (abs(fb) >= 2) {
            ...
            if (fb >= 2) { 
                if (alt_NO((*atual_ptr)->esq->esq) >= alt_NO((*atual_ptr)->esq->dir))
                    rotacaoLL(atual_ptr);
                else
                    rotacaoLR(atual_ptr);
            } else { 
                if (alt_NO((*atual_ptr)->dir->dir) >= alt_NO((*atual_ptr)->dir->esq))
                    rotacaoRR(atual_ptr);
                else
                    rotacaoRL(atual_ptr);
            }
